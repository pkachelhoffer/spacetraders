...
```golang
import stclient "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
```golang
ctx := context.WithValue(context.Background(), stclient.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), stclient.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), stclient.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), stclient.ContextOperationServerVariables, map[string]map[string]string{
